...
class W1HtmlAmiro extends \WalletOne\Classes\StandardsMethods\W1Html  {
        ...
        foreach ($list['data'] as $sublist) {
          if (!empty($sublist['data'])) {
            ...
            foreach ($sublist['data'] as $key => $subsublist) {
              ...
              $html .=  'onchange="changeFields(this, '.($paymentName == 'WMI_PTENABLED' ? 1 : 2).')">';
            ...
            $html .= 'value="' . $sublist['id'] . '" onchange="changeFields(this, '.($paymentName == 'WMI_PTENABLED' ? 1 : 2).')">';
            $html .= '<label for="'.$nameId.'">'
